    ...
    <div style="display: none" id="forms">
        <form action="{{ route('garage.store') }}" method="post">
            ...
            <div class="row">
                <div class="col">
                    <div class="form-group">
                        <label for="recipient-name" class="col-form-label">{{ __('plafon') }}</label>
            ...
            <div class="row">
                <div class="col">
                    <div class="form-group">
                        <label for="recipient-name" class="col-form-label">{{ __('number_rows') }}</label>
                        <input type="text" class="form-control" name="rows">
                    </div>
                </div>
                <div class="col">
                    <div class="form-group">
                        <label for="recipient-name" class="col-form-label">{{ __('number_columns') }}</label>
        ...
        <table id="copy-print-csv" class="table custom-table">
            <thead>
                <tr>
                    ...
                    <th>{{ __('plafon') }}</th>
                    <th>{{ __('address') }}</th>
                    <th>{{ __('number_rows') }}</th>
                    <th>{{ __('number_columns') }}</th>
                    <th>{{ __('edit') }}</th>
                ...
                @foreach ($data as $d)
                    <tr>
                        ...
                        <td><a href="#" class="btn btn-primary btn-sm edit-garage" data-toggle="modal" data-target="#customModalTwo" data-id="{{ $d->id }}">{{ __('edit') }}</a>
                            ...
                            <form action="{{ route('garage.destroy', $d->id) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
            ...
            $('.edit-garage').click(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#ups').attr('action', '/garage/' + id);
                        $('#ups').append('<input type="hidden" name="_method" value="PUT">');
                        $('input[name="plafon"]').val(data.plafon);
